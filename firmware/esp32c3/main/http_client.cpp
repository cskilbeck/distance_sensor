                       ...
                       void const *post_data, size_t post_size)
{
    ...
    LOG_INFO("%s %s", HTTP_METHOD_MAPPING[method], url);
    ...
    if(client == nullptr) {
        LOG_ERROR("esp_http_client_init failed");
        return ESP_FAIL;
    }
    ...
    *response_code = esp_http_client_get_status_code(client);
    ...
    if(*response_code >= 300) {
        LOG_WARN("Status = %d", *response_code);
    }

    LOG_VERBOSE("Response: %s", response_buffer);
