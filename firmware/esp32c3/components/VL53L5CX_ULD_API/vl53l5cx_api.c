...
static uint8_t _vl53l5cx_send_offset_data(VL53L5CX_Configuration *p_dev, uint8_t resolution)
{
    ...
    memcpy(p_dev->temp_buffer, p_dev->offset_data, VL53L5CX_OFFSET_BUFFER_SIZE);

    /* Data extrapolation is required for 4X4 offset */
    if(resolution == (uint8_t)VL53L5CX_RESOLUTION_4X4) {
        memcpy(&(p_dev->temp_buffer[0x10]), dss_4x4, sizeof(dss_4x4));
        SwapBuffer(p_dev->temp_buffer, VL53L5CX_OFFSET_BUFFER_SIZE);
        memcpy(signal_grid, &(p_dev->temp_buffer[0x3C]), sizeof(signal_grid));
        memcpy(range_grid, &(p_dev->temp_buffer[0x140]), sizeof(range_grid));

        for(j = 0; j < (int8_t)4; j++) {
            for(i = 0; i < (int8_t)4; i++) {
                signal_grid[i + (4 * j)] = (signal_grid[(2 * i) + (16 * j) + (int8_t)0] + signal_grid[(2 * i) + (16 * j) + (int8_t)1] +
                                            signal_grid[(2 * i) + (16 * j) + (int8_t)8] + signal_grid[(2 * i) + (16 * j) + (int8_t)9]) /
                                           (uint32_t)4;
                range_grid[i + (4 * j)] = (range_grid[(2 * i) + (16 * j)] + range_grid[(2 * i) + (16 * j) + 1] + range_grid[(2 * i) + (16 * j) + 8] +
                                           range_grid[(2 * i) + (16 * j) + 9]) /
                                          (int16_t)4;
            }
        }
        memset(&range_grid[0x10], 0, (uint16_t)96);
        memset(&signal_grid[0x10], 0, (uint16_t)192);
        memcpy(&(p_dev->temp_buffer[0x3C]), signal_grid, sizeof(signal_grid));
        memcpy(&(p_dev->temp_buffer[0x140]), range_grid, sizeof(range_grid));
        SwapBuffer(p_dev->temp_buffer, VL53L5CX_OFFSET_BUFFER_SIZE);
    }
    ...
    memcpy(&(p_dev->temp_buffer[0x1E0]), footer, 8);
...
static uint8_t _vl53l5cx_send_xtalk_data(VL53L5CX_Configuration *p_dev, uint8_t resolution)
{
    ...
    memcpy(p_dev->temp_buffer, &(p_dev->xtalk_data[0]), VL53L5CX_XTALK_BUFFER_SIZE);

    /* Data extrapolation is required for 4X4 Xtalk */
    if(resolution == (uint8_t)VL53L5CX_RESOLUTION_4X4) {
        memcpy(&(p_dev->temp_buffer[0x8]), res4x4, sizeof(res4x4));
        memcpy(&(p_dev->temp_buffer[0x020]), dss_4x4, sizeof(dss_4x4));

        SwapBuffer(p_dev->temp_buffer, VL53L5CX_XTALK_BUFFER_SIZE);
        memcpy(signal_grid, &(p_dev->temp_buffer[0x34]), sizeof(signal_grid));

        for(j = 0; j < (int8_t)4; j++) {
            for(i = 0; i < (int8_t)4; i++) {
                signal_grid[i + (4 * j)] = (signal_grid[(2 * i) + (16 * j) + 0] + signal_grid[(2 * i) + (16 * j) + 1] + signal_grid[(2 * i) + (16 * j) + 8] +
                                            signal_grid[(2 * i) + (16 * j) + 9]) /
                                           (uint32_t)4;
            }
        }
        memset(&signal_grid[0x10], 0, (uint32_t)192);
        memcpy(&(p_dev->temp_buffer[0x34]), signal_grid, sizeof(signal_grid));
        SwapBuffer(p_dev->temp_buffer, VL53L5CX_XTALK_BUFFER_SIZE);
        memcpy(&(p_dev->temp_buffer[0x134]), profile_4x4, sizeof(profile_4x4));
        memset(&(p_dev->temp_buffer[0x078]), 0, (uint32_t)4 * sizeof(uint8_t));
    }
...
uint8_t vl53l5cx_is_alive(VL53L5CX_Configuration *p_dev, uint8_t *p_is_alive)
{
    ...
    *p_is_alive = ((device_id == (uint8_t)0xF0) && (revision_id == (uint8_t)0x02));
...
uint8_t vl53l5cx_init(VL53L5CX_Configuration *p_dev)
{
    ...
    memcpy(p_dev->offset_data, p_dev->temp_buffer, VL53L5CX_OFFSET_BUFFER_SIZE);
    status |= _vl53l5cx_send_offset_data(p_dev, VL53L5CX_RESOLUTION_4X4);

    /* Set default Xtalk shape. Send Xtalk to sensor */
    memcpy(p_dev->xtalk_data, (uint8_t *)VL53L5CX_DEFAULT_XTALK, VL53L5CX_XTALK_BUFFER_SIZE);
    ...
    tmp = (uint8_t)1;
    status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_GLARE_FILTER, 40, (uint8_t *)&tmp, 1, 0x26);
    status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_GLARE_FILTER, 40, (uint8_t *)&tmp, 1, 0x25);
...
uint8_t vl53l5cx_start_ranging(VL53L5CX_Configuration *p_dev)
{
    ...
    uint32_t output[] = { VL53L5CX_START_BH,      VL53L5CX_METADATA_BH,           VL53L5CX_COMMONDATA_BH,    VL53L5CX_AMBIENT_RATE_BH,
                          VL53L5CX_SPAD_COUNT_BH, VL53L5CX_NB_TARGET_DETECTED_BH, VL53L5CX_SIGNAL_RATE_BH,   VL53L5CX_RANGE_SIGMA_MM_BH,
                          VL53L5CX_DISTANCE_BH,   VL53L5CX_REFLECTANCE_BH,        VL53L5CX_TARGET_STATUS_BH, VL53L5CX_MOTION_DETECT_BH };
...
uint8_t vl53l5cx_check_data_ready(VL53L5CX_Configuration *p_dev, uint8_t *p_isReady)
{
    ...
    if((p_dev->temp_buffer[0] != p_dev->streamcount) && (p_dev->temp_buffer[0] != (uint8_t)255) && (p_dev->temp_buffer[1] == (uint8_t)0x5) &&
       ((p_dev->temp_buffer[2] & (uint8_t)0x5) == (uint8_t)0x5) && ((p_dev->temp_buffer[3] & (uint8_t)0x10) == (uint8_t)0x10)) {
        *p_isReady = (uint8_t)1;
...
uint8_t vl53l5cx_get_ranging_data(VL53L5CX_Configuration *p_dev, VL53L5CX_ResultsData *p_results)
{
    ...
    for(i = (uint32_t)16; i < (uint32_t)p_dev->data_read_size; i += (uint32_t)4) {
        ...
        case VL53L5CX_AMBIENT_RATE_IDX:
            memcpy(p_results->ambient_per_spad, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_NB_SPADS_ENABLED
        case VL53L5CX_SPAD_COUNT_IDX:
            memcpy(p_results->nb_spads_enabled, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_NB_TARGET_DETECTED
        case VL53L5CX_NB_TARGET_DETECTED_IDX:
            memcpy(p_results->nb_target_detected, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_SIGNAL_PER_SPAD
        case VL53L5CX_SIGNAL_RATE_IDX:
            memcpy(p_results->signal_per_spad, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_RANGE_SIGMA_MM
        case VL53L5CX_RANGE_SIGMA_MM_IDX:
            memcpy(p_results->range_sigma_mm, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_DISTANCE_MM
        case VL53L5CX_DISTANCE_IDX:
            memcpy(p_results->distance_mm, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_REFLECTANCE_PERCENT
        case VL53L5CX_REFLECTANCE_EST_PC_IDX:
            memcpy(p_results->reflectance, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_TARGET_STATUS
        case VL53L5CX_TARGET_STATUS_IDX:
            memcpy(p_results->target_status, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
#ifndef VL53L5CX_DISABLE_MOTION_INDICATOR
        case VL53L5CX_MOTION_DETEC_IDX:
            memcpy(&p_results->motion_indicator, &(p_dev->temp_buffer[i + (uint32_t)4]), msize);
            break;
#endif
        default:
            break;
...
uint8_t vl53l5cx_get_integration_time_ms(VL53L5CX_Configuration *p_dev, uint32_t *p_time_ms)
{
    ...
    memcpy(p_time_ms, &(p_dev->temp_buffer[0x0]), 4);
...
uint8_t vl53l5cx_enable_internal_cp(VL53L5CX_Configuration *p_dev)
{
    ...
    status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_INTERNAL_CP, 16, (uint8_t *)&vcsel_bootup_fsm, 1, 0x0A);

    status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer, VL53L5CX_DCI_INTERNAL_CP, 16, (uint8_t *)&analog_dynamic_pad_0, 1, 0x0E);
...
uint8_t vl53l5cx_dci_write_data(VL53L5CX_Configuration *p_dev, uint8_t *data, uint32_t index, uint16_t data_size)
{
    ...
    uint8_t footer[] = { 0x00, 0x00, 0x00, 0x0f, 0x05, 0x01, (uint8_t)((data_size + (uint16_t)8) >> 8), (uint8_t)((data_size + (uint16_t)8) & (uint8_t)0xFF) };
    ...
    if((data_size + (uint16_t)12) > (uint16_t)VL53L5CX_TEMPORARY_BUFFER_SIZE) {
        ...
        memcpy(&p_dev->temp_buffer[0], headers, sizeof(headers));
        memcpy(&p_dev->temp_buffer[data_size + (uint16_t)4], footer, sizeof(footer));
...
uint8_t vl53l5cx_dci_replace_data(VL53L5CX_Configuration *p_dev, uint8_t *data, uint32_t index, uint16_t data_size, uint8_t *new_data, uint16_t new_data_size,
                                  uint16_t new_data_pos)
{
    uint8_t status = VL53L5CX_STATUS_OK;

    status |= vl53l5cx_dci_read_data(p_dev, data, index, data_size);
    memcpy(&(data[new_data_pos]), new_data, new_data_size);
